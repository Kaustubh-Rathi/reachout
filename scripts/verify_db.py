...
print("attempts with NULL sender_account_id:", q("select count(*) from outreach_attempts where sender_account_id is null")[0][0])
print("duplicate phones in contacts:", q("select phone,count(*) from contacts where phone is not null group by phone having count(*)>1"))
print("unreachable contacts:", q("select count(*) from contacts where (phone is null or phone='') and (email is null or email='')")[0][0])
